...
if response.success?
flash[:notice]="Thank You for using CoffeeHouse. The oreder details are sent to your mail"
session[:cart_id]=nil
...
def search
  ...
  @coffees = Store.find_by_sql("select * from stores WHERE name like '%#{params[:query]}%' or size like '%#{params[:query]}%' or description like '%#{params[:query]}%'")
end
